...
def create_model(np.ndarray[DTYPE_t, ndim=2] data, int n_bins, float lamb):
...
def test_model(np.ndarray[DTYPE_t, ndim=2] test_data, np.ndarray[double, ndim=2] model, np.ndarray[double, ndim=1] h_i, np.ndarray[DTYPE_t, ndim=1] expected, float cutoff, int n_bins):
