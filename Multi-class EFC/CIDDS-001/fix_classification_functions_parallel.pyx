...
def create_multiclass_model_parallel(np.ndarray[DTYPE_t, ndim=2] data, np.ndarray[DTYPE_t, ndim=1] labels, int n_bins, float lamb):
    ...
    rounds = [x for x in range(n_classes//n_jobs)]

    for i in rounds:
        with concurrent.futures.ProcessPoolExecutor() as executor:
            results = executor.map(create_multiclass_model, data_per_type[i*n_jobs:(i+1)*n_jobs], n_jobs*[n_bins], n_jobs*[lamb])

        for indx, result in enumerate(results):
            h_i_matrices[indx+i] = result[0]
            coupling_matrices[indx+i] = result[1]
            cutoffs_list[indx+i] = result[2]

    cdef int remain = n_classes % n_jobs
    
    if remain != 0:
        with concurrent.futures.ProcessPoolExecutor() as executor:
            results = executor.map(create_multiclass_model, data_per_type[-remain:], remain*[n_bins], remain*[lamb])

        for indx, result in enumerate(results):
            h_i_matrices[-remain+indx] = result[0]
            coupling_matrices[-remain+indx] = result[1]
            cutoffs_list[-remain+indx] = result[2]
