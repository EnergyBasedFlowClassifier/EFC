    ...
    with open("5-fold_sets/Results/Algorithms_comparison.txt", "w+") as file:
        ...
        for idx, alg in enumerate(["EFC", "NB", "KNN", "DT", "SVC", "MLP", "RF"]):
            ...
            for sets in range(1, 6):
                ...
                f1.append(
                    f1_score(y_true, y_pred, average=None, labels=[0, 1, 2, 3, 4])
                )
...
def plot_unknown():
    ...
    for alg in ["SVC", "EFC"]:
        ...
        if alg != "EFC":
            ...
            ax[1].set_title("EFC")
        fig.savefig(
            "5-fold_sets/Results/EFC_SVC_unknown_CIDDS001.pdf",
            format="pdf",
            bbox_inches="tight",
        )  #
...
def plot_unknown_on_top():
    ...
    for alg in ["RF", "EFC"]:
        ...
        fig.savefig(
            "5-fold_sets/Results/EFC_RF_unknown_CIDDS001.pdf",
            format="pdf",
            bbox_inches="tight",
        )  #
...
def times():
    for alg in ["RF", "NB", "KNN", "SVC", "MLP", "AD", "DT", "EFC"]:
        ...
        for i in range(1, 6):
            times = np.load(
                "5-fold_sets/Results/Sets{}/{}_times.npy".format(i, alg),
                allow_pickle=True,
            )
            train.append(times[0])
            test.append(times[1])
        print(
            "{} & {:.3f} $\\pm$ {:.3f} & {:.3f} $\\pm$ {:.3f} \\\\".format(
                alg,
                mean(train),
                1.96 * stdev(train) / sqrt(len(train)),
                mean(test),
                1.96 * stdev(test) / sqrt(len(test)),
            )
        )
