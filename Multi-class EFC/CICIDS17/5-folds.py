...
def cv_discretized():
    ...
    reader = pd.read_csv("TrafficLabelling /Discretized.csv", chunksize=4000000, header=None)
    for chunk in reader:
...
def cv_non_discretized():
    ...
    reader_pre = pd.read_csv("TrafficLabelling /Pre_processed_encoded.csv", chunksize=3000000, header=None)
    n_chunk = 0
